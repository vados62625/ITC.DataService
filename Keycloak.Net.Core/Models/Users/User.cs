...
namespace Keycloak.Net.Core.Models.Users
{
    public class User
    {
        ...
        public Guid? GetEmployeeId()
        {
            ...
            var employeeidAttribute = Attributes.FirstOrDefault(attribute => attribute.Key == "employeeid");
            if (employeeidAttribute.Key == null || employeeidAttribute.Value == null)
                return null;

            var employeeId = employeeidAttribute.Value.FirstOrDefault();

            Guid.TryParse(employeeId, out var result);
